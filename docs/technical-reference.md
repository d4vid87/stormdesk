...
# StormDesk technical reference

This guide keeps the detailed setup, integration, hosting, and development notes out of the main
[README](../README.md). Start there if you only want to install and use StormDesk.
...
- **Linux** — `sudo apt install ./StormDesk_*_amd64.deb` (Debian, Ubuntu, Mint, Pop!_OS). Other
  64-bit distributions can use `StormDesk_*_amd64.AppImage`; add `--browser` if the desktop window
  is blank on that graphics setup.
...
leave the old one there. See the [Home Assistant guide](homeassistant.md).
